...
export const bounce: AnimationTriggerMetadata =
  trigger('bounce', [
    state('in', style({ transform: 'translate({{inX}}vw, {{inY}}vh)', opacity: 0.3 }), { params: { inX: 100, inY: 100 }}),
    state('out', style({ transform: 'translate({{x}}vw, {{y}}vh)', opacity: 0.3 }), { params: { x: 100, y: 100 } }),
